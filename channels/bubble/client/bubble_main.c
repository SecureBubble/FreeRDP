...
static void bubble_handle_process_created(BubbleClientContext* context, wStream* s)
{
    UINT32 proc_id;
    UINT64 timestamp;
    UINT error = CHANNEL_RC_OK;
    ...
    WLog_INFO(TAG, "new process: time=%d, proc_id=%d, proc_name=%s, cmdline=%s", timestamp,
                 proc_id, proc_name, cmdline);
...
static void bubble_handle_active_window_changed(BubbleClientContext* context, wStream* s)
{
    UINT error = CHANNEL_RC_OK;
...
static void bubble_handle_keep_alive(BubbleClientContext* context, wStream* s)
{
    UINT64 timestamp;
    UINT error = CHANNEL_RC_OK;
...
static void bubble_handle_input_focus_change(BubbleClientContext* context, wStream* s)
{
    BOOL is_password;
    UINT error = CHANNEL_RC_OK;
...
static void bubble_handle_uac_window_state(BubbleClientContext* context, wStream* s)
{
    BOOL is_uac_shown;
    UINT error = CHANNEL_RC_OK;
...
static void bubble_request_exec_app(BubbleClientContext* context)
{
	UINT error = CHANNEL_RC_OK;
	...
	WLog_INFO(TAG, "remote app mode: %d", settings->RemoteApplicationMode);
		...
		WLog_INFO(TAG, "requesting agent to execute %s [len=%d]", app_to_execute, strlen(app_to_execute));
		WLog_INFO(TAG, "lbinfolen=%d", settings->LoadBalanceInfoLength);

                data_in = Stream_New(NULL, 2 + 2 + strlen(app_to_execute) + 2 + strlen(settings->Username) + 2 + settings->LoadBalanceInfoLength);
	...
	winpr_HexDump(TAG, WLOG_INFO, Stream_Buffer(data_in), Stream_Length(data_in));
	
	WLog_INFO(TAG, "sending exec app request");
	bubble_send(plugin, data_in);
	//Stream_Free(data_in, TRUE);

}

static void bubble_handle_query_mode(BubbleClientContext* context, wStream* s)
{
	UINT error = CHANNEL_RC_OK;
    WLog_INFO(TAG, "%s", __FUNCTION__);
    bubblePlugin* plugin = (bubblePlugin*)context->handle;
	...
	WLog_INFO(TAG, "responding to query mode");
	bubble_send(plugin, data_in);
	//Stream_Free(data_in, TRUE);
}

static void bubble_handle_disconnection_request(BubbleClientContext* context, wStream* s)
{
	UINT error = CHANNEL_RC_OK;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT bubble_virtual_channel_event_connected(bubblePlugin* bubble, LPVOID pData, UINT32 dataLength)
{
	...
	bubble->MsgsHandle = channel_client_create_handler(bubble->rdpcontext, bubble, bubble_order_recv,
	                                                 "bubble");
	...
	switch (event)
	{
		case CHANNEL_EVENT_CONNECTED:
			if ((error = bubble_virtual_channel_event_connected(bubble, pData, dataLength)))
				WLog_ERR(TAG, "bubble_virtual_channel_event_connected failed with error %" PRIu32 "!",
				         error);
...
BOOL VCAPITYPE VirtualChannelEntryEx(PCHANNEL_ENTRY_POINTS pEntryPoints, PVOID pInitHandle)
{
	...
	CopyMemory(&(bubble->channelEntryPoints), pEntryPoints, sizeof(CHANNEL_ENTRY_POINTS_FREERDP_EX));
