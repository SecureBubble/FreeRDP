...
static int shadow_client_subsystem_process_message(rdpShadowClient* client, wMessage* message)
{
	...
	switch (message->id)
	{
		...
		case SHADOW_MSG_UDP_CONNECTION_ID:
		{
			...
			if (!peer->AssociateUdpConn(peer, msg->channel)) {
				WLog_ERR(TAG, "error associating udp peer");
			}
...
BOOL findPeer(void* data, size_t index, va_list ap)
{
	...
	WLog_DBG(TAG, "found match with shadowClient %s", freerdp_settings_get_string(client->context.settings, FreeRDP_ClientHostname));
