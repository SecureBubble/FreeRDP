...
static BOOL pf_server_start_peer(freerdp_peer* client)
{
	...
	HANDLE hThread = CreateThread(nullptr, 0, pf_server_handle_peer, args, CREATE_SUSPENDED, nullptr);
	if (!hThread)
	{
		free(args);
		return FALSE;
...
BOOL pf_server_start_with_peer_socket(proxyServer* server, int socket)
{
	...
	freerdp_peer* client = client = freerdp_peer_new(socket);
	if (!client)
		goto fail;
...
proxyServer* pf_server_new(const proxyConfig* config)
{
	...
	rdpSettings* listenerSettings = nullptr;
