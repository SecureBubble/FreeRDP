...
#define MAX_PING_DELAY 200
#define MAX_ACK_INTERVAL 20
#define MAX_ACKVEC_INTERVAL 20
#define LOST_CHECK_INTERVAL 200
...
void recompute_nextAckDates(Udp3Context* udp3, UINT64 now, UINT64 *nextAck, UINT64 *nextAckVec)
{
	Udp3ReceiveWindow *w = udp3->recvWindow;
	BOOL isFirst = TRUE;
	BOOL firstReceived = TRUE;

	UINT64 candNextAck = NEXT_TIME_NEVER;
	UINT64 candNextAckVec = NEXT_TIME_NEVER;

	for (UINT64 i = w->lowIndex; i < w->upIndex; i++)
	{
		const ReceivedPacket *p = &w->window[i % w->windowSize];

		if (isFirst)
		{
			firstReceived = (p->state == UDP2_PACKET_RECEIVED);
			isFirst = FALSE;
		}

		if (p->state == UDP2_PACKET_RECEIVED)
		{
			UINT64 cand = p->recvTimestamp + udp3->remoteDelayedAckTimeout;

			if (!firstReceived)
			{
				if (cand < candNextAckVec)
					candNextAckVec = cand;
			}
			else if (i <= w->ackableIndex)
			{
				if (cand < candNextAck)
					candNextAck = cand;
			}
	...
	if ((w->ackableIndex >= w->lowIndex) && (1 + w->ackableIndex - w->lowIndex > udp3->remoteMaxDelayedAcks))
		candNextAck = now;
	...
	*nextAck = candNextAck;
	...
	if (!firstReceived)
	{
		/* look at the channel window because if the first packet is missing, we want to boost the
		 * sending of a ackvec because it probably means that this packet was lost
		 */
		if (!w->channelWindow[w->lowChannelWindow % w->windowSize].s)
		{
			UINT64 cand = now + 2;
			if (cand < candNextAckVec)
				candNextAckVec = cand;
	...
	if (w->upIndex - w->lowIndex < 7)
		candNextAckVec = NEXT_TIME_NEVER;

	*nextAckVec = candNextAckVec;

	char nextAckStr[50];
	if (candNextAck != NEXT_TIME_NEVER)
		snprintf(nextAckStr, sizeof(nextAckStr), "%ld", (signed long)candNextAck-now);
	else
		strcpy(nextAckStr, "<never>");

	char nextAckVecStr[50];
	if (candNextAckVec != NEXT_TIME_NEVER)
		snprintf(nextAckVecStr, sizeof(nextAckVecStr), "%ld", (signed long)candNextAckVec - now);
	else
		strcpy(nextAckVecStr, "<never>");
	UDP_DEBUG("[%lx - %lx] ackable=0x%lx nextAck=%s nextAckVec=%s", w->lowIndex, w->upIndex, w->ackableIndex, nextAckStr, nextAckVecStr);
}


static BOOL pushAckVec(Udp3Context* udp3, wStream* s, UINT64 now)
{
	Udp3ReceiveWindow* recvW = udp3->recvWindow;

	wStream payloadStaticStream;
	wStream* payloadStream =
	    Stream_StaticInit(&payloadStaticStream, Stream_Pointer(s), Stream_GetRemainingLength(s));
	Stream_Seek(payloadStream, 2 + 1); /* BaseSeqNum + (codedAckVecSize|TimeStampPresent) */

	BOOL addTimestamp = !udp3->firstAckVec;
	if (addTimestamp)
		Stream_Seek(payloadStream, 3 + 1); /* TimeStamp + SendAckTimeGapInMs */
	...
	while (i < recvW->upIndex && nAckItems < 127)
	{
		...
		if (rleMode)
		{
			...
			if (lastValue)
				b |= 0x40;
			Stream_Write_UINT8(payloadStream, b);
			UDP_DEBUG(" * rle(%d) len=%d [0x%lx -> 0x%lx]", lastValue, rleLen, i, i + rleLen);
			i += rleLen;
		}
		else
		{
			UDP_DEBUG(" * bitmap=0x%x [0x%lx -> 0x%lx]", currentBitMask, i, i + 6);
			Stream_Write_UINT8(payloadStream, currentBitMask);
			i += 7;
		}
	...
	Stream_Write_UINT8(s, nAckItemsflags);
	if (addTimestamp)
	{
		Stream_Write_UINT24(s,
		                    recvW->window[recvW->ackableIndex % recvW->windowSize].recvTimestamp);
		Stream_Write_UINT8(s, 0xff); /* SendAckTimeGapInMs: 0xff => invalid */
	}

	Stream_Seek(s, Stream_GetPosition(payloadStream));

	if (udp3->firstAckVec)
		udp3->firstAckVec = FALSE;

	recompute_nextAckDates(udp3, now, &udp3->nextAckTime, &udp3->nextAckVecTime);
	udp3->lastAckVecTime = now;
	return TRUE;
}
...
static void pushAck(Udp3Context* udp3, wStream* s, UINT64 now)
{
	...
	if (nacks)
	{
		...
		UDP_DEBUG("range=[0x%.6lx--0x%.6lx] ackable=0x%.6lx acking16=0x%.04x (%d acks)",
		          recvW->lowIndex, recvW->upIndex, recvW->ackableIndex, acked16, nacks);
		...
		udp3->lastAckSent = recvW->lowIndex - 1;
	}
	else
	{
...
static BOOL shouldAck(Udp3Context* udp3, UINT64 now)
{
	return (udp3->nextAckTime < now) && (udp3->lastAckSent < udp3->recvWindow->ackableIndex); //(udp3->lastAckTime + MAX_ACK_INTERVAL <= now);
}

static BOOL shouldAckVec(Udp3Context* udp3, UINT64 now)
{
	return (udp3->nextAckVecTime < now) && (udp3->lastAckVecTime + MAX_ACKVEC_INTERVAL <= now);

}

static void udp3_write_headersEx(wStream* s, Udp3Context* udp3, UINT64 now, UdpDataType dataType, UINT16 dataSeq, UINT16 flags)
{
	...
	Stream_Write_UINT16(s, flags | (udp3->logWindowSize << 12));
	...
	{
		Stream_Write_UINT8(s, 8); // TODO: hardcoded for now
	...
	if (flags & RDPUDP2_ACKVEC)
	{
		...
		if (!pushAckVec(udp3, s, now)) {
			WLog_ERR(TAG, "strange !!!!");
		}
	}
}

static void udp3_write_headers(wStream* s, Udp3Context* udp3, UdpDataType dataType, UINT16 dataSeq,
        BOOL piggyBack, UINT16 *pflags)
{
	...
	if (piggyBack && shouldAck(udp3, now))
		flags |= RDPUDP2_ACK;
	...
	if (piggyBack && !(flags & RDPUDP2_ACK) && shouldAckVec(udp3, now))
		flags |= RDPUDP2_ACKVEC;

	udp3_write_headersEx(s, udp3, now, dataType, dataSeq, flags);
	if (pflags)
		*pflags = flags;
}
...
BOOL Udp3ReceiveWindow_updateAckable(Udp3ReceiveWindow* recvw, UINT64 now)
{
	UINT64 newAckable = (recvw->lowIndex - 1);

	for (UINT64 seqIndex = recvw->lowIndex; seqIndex < recvw->upIndex; seqIndex++)
	{
		if (recvw->window[seqIndex % recvw->windowSize].state != UDP2_PACKET_RECEIVED)
			break;
		newAckable = seqIndex;
	}

	if (newAckable > recvw->ackableIndex)
	{
		UDP_DEBUG("new ackable seqId=0x%.04lx", newAckable);
		recvw->ackableIndex = newAckable;
		return TRUE;
	}
	return FALSE;
}
...
BOOL Udp3ReceiveWindow_recvDataPacket(Udp3ReceiveWindow* w, UINT16 seqNumber16,
                                      UINT16 channelSeqNumber16, wStream* payload, UINT64 now)
{
	char payloadMsg[150];
	...
	if (payload)
		snprintf(payloadMsg, sizeof(payloadMsg) - 1,
		         "chanSeqNumber=0x%lx(chanData16=0x%x) chanWindow=[%.8lx-%.8lx] payloadSz=%ld",
	...
	UDP_DEBUG("seqNumber=0x%lx %s", seqNumber, payloadMsg);
	if (seqNumber < w->lowIndex)
	{
		WLog_INFO(TAG, "packet is out-of-window, lowIndex=0x%lx seqNumber=0x%lx", w->lowIndex,
		          seqNumber);
		return FALSE;
	}
	...
	rpacket->recvTimestamp = now;

	Udp3ReceiveWindow_updateAckable(w, now);
	...
	{
		WLog_ERR(TAG,
		         "chunk 0x%.8lx(chanSeqNumber16=0x%x) is out of channelWindow [0x%.8lx -> "
		         "0x%.8lx], retransmit ?",
		         channelSeqNumber, channelSeqNumber16, w->lowChannelWindow, w->upChannelWindow);
		return TRUE;
	}
	...
	if (channelSeqNumber == w->lowChannelWindow)
	{
		for (; w->lowChannelWindow <= w->upChannelWindow; w->lowChannelWindow++)
		{
			...
			UDP_DEBUG("pushing chanSeqId=0x%lx recvSeqId=0x%lx", w->lowChannelWindow,
...
BOOL Udp3ReceiveWindow_treatAoA(Udp3ReceiveWindow* w, UINT16 aoa16, UINT64 now)
{
	UINT64 aoa = computeFullSeqNumber(w->lowIndex, aoa16);

	UDP_DEBUG("aoa16=0x%x [0x%lx - 0x%lx]", aoa16, w->lowIndex, w->upIndex);
	if (aoa < w->lowIndex)
	{
                               ...
                               wStream* payload)
{
	...
	/* find a free slot */
	for (size_t i = 0; i < integrity->reservedSize; i++)
	{
...
{
	return (InterlockedIncrement(&w->upIndex) - 1) & 0xffff;
}

UINT16 Udp3SenderWindow_reserveChannelSeq(Udp3SenderWindow* w)
{
	UINT64 ret = InterlockedIncrement(&w->channelSeqNumber) - 1;
...
BOOL Udp3SenderWindow_Push(Udp3SenderWindow* w, UINT16 dataSeq, UINT16 channelSeq, wStream* payload)
{
	...
	node->channelSeqId = channelSeq;
	return IntegrityManager_register(&w->integrity, dataSeq, channelSeq, payload);
}

void Udp3SenderWindow_recomputeLowBound(Udp3SenderWindow* w, UINT64 now)
{
	UINT64 newLowIndex;
...
outLoop:
	...
	{
		UDP_DEBUG("sender window updating lowIndex from 0x%.8lx to 0x%.8lx", w->lowIndex,
		          newLowIndex);
		w->pendingAoA = TRUE;
...
BOOL Udp3SenderWindow_treatAck(Udp3SenderWindow* w, Udp3Packet* p, UINT64 now)
{
	UINT64 targetSeqNumber = computeFullSeqNumber(w->lowIndex, p->ackSeqNumber);

	UDP_DEBUG("Ack(base=0x%lx down/up Index=[0x%lx-0x%x])", targetSeqNumber, w->lowIndex,
	          w->upIndex);
	if (targetSeqNumber < w->lowIndex || targetSeqNumber > w->upIndex)
	{
		UDP_DEBUG("seqNumber 0x%lx out of range (0x%lx -> 0x%x)", targetSeqNumber, w->lowIndex,
		          w->upIndex);
		w->pendingAoA = TRUE;
		return FALSE;
	}

	for (UINT64 seqIndex = targetSeqNumber; seqIndex >= w->lowIndex; seqIndex--)
	{
	...
	Udp3SenderWindow_recomputeLowBound(w, now);
	return TRUE;
}
...
BOOL udp3_resend_packet(rdpUdpTransport* udp, Udp3Context* udp3, UINT64 seqId)
{
	...
	UDP_DEBUG("resending chanDataSeq=0x%lx origSeqId=0x%x newSeqId=0x%x", packet->channelSeqId,
	          origSeqId, packet->seqId);

	udp3_write_headers(s, udp3, UDPDATA_TYPE_DATA, packet->seqId, FALSE, nullptr);
	...
	udp3->nextPing = GetTickCount64() + 50;

	return ret;
}

BOOL Udp3SenderWindow_treatAckvec(rdpUdpTransport* udp, Udp3Context* udp3, Udp3Packet* p, UINT64 now)
{
	Udp3SenderWindow* w = &udp3->sendingWindow;
	UINT64 seq = computeFullSeqNumber(w->lowIndex, p->ackVecBaseSeqNum);

	UDP_DEBUG("AckVec(base=0x%lx down/up Index=[0x%lx-0x%x])", seq, w->lowIndex, w->upIndex);

	if (seq < w->lowIndex || w->upIndex < p->ackVecBaseSeqNum)
	{
		UDP_DEBUG("seqNumber 0x%x out of range (0x%lx -> 0x%x)", p->ackVecBaseSeqNum, w->lowIndex,
		          w->upIndex);
	}

	for (BYTE i = 0; i < p->codedAckVecSize; i++)
	{
		Udp3Node* node;
		...
		if (b & 0x80)
		{
			...
			for (size_t j = 0; j < runLength; j++, seq++)
			{
				...
				switch (node->state)
				{
					...
					case UDP3_PACKET_RECEIVED:
						if (!status)
							WLog_ERR(TAG, "Ignoring lost status for already acked packet 0x%lx",
							         seq);
						break;
					case UDP3_PACKET_LOST:
						if (status)
				...
				{
					UDP_DEBUG("seq packet 0x%lx not in integrity manager", seq);
				}
			}
		}
		else
		{
			/* bitmap mode */
			BYTE mask = 1;
			for (size_t j = 0; j < 7; j++, mask <<= 1, seq++)
			{
				if (seq < w->lowIndex) /* lower than low limit */
					continue;
				...
				switch (node->state)
				{
					...
					case UDP3_PACKET_RECEIVED:
						if (!status)
							WLog_DBG(TAG, "Ignoring lost status for already acked packet 0x%lx", seq);
						break;
					case UDP3_PACKET_LOST:
						if (status)
						{
				...
				if (dropData && !IntegrityManager_dispose(&w->integrity, seq % w->windowSize))
				{
					UDP_DEBUG("%s: seq packet 0x%lx not in integrity manager", __func__, seq);
				}
			}
		}
	} /* for */
	...
	/* note: limit us to the last treated sequence number, otherwise we will resend packets that
	 * were not concerned by the received ACKVEC
	 */
	for (UINT64 j = w->lowIndex; j < seq; j++)
	{
		...
		{
			WLog_ERR(TAG, "error resending packet original seqId=0x%lx", j);
		}
	}
	return TRUE;
}
...
BOOL udp3_init(rdpUdpTransport* udp, Udp3Context* udp3, BOOL server)
{
	...
	udp3->localMaxDelayedAcks = 35;
	udp3->localDelayedAckTimeout = 20;
	udp3->timebase = GetTickCount64();
	udp3->nextLostCheck = udp3->timebase + LOST_CHECK_INTERVAL;
	udp3->firstAckVec = TRUE;
	udp3->remoteMaxDelayedAcks = 8;
	udp3->remoteDelayedAckTimeout = 100;
	udp3->lastAckTime = 0;
	udp3->nextAckTime = NEXT_TIME_NEVER;
	udp3->lastAckSent = 0;
	udp3->lastAckVecTime = 0;
	udp3->nextAckVecTime = NEXT_TIME_NEVER;

	InitializeCriticalSection(&udp3->sendingLock);
...
void udp3_destroy(Udp3Context* udp3)
{
	...
	DeleteCriticalSection(&udp3->sendingLock);
...
static BOOL checkForAckOrAckVec(rdpUdpTransport* udp, Udp3Context *udp3, UINT64 now)
{
	if (!shouldAck(udp3, now) && !shouldAckVec(udp3, now))
		return TRUE;

	wStream *packet = StreamPool_Take(udp->packetPool, udp->mtus.up);
	if (!packet)
		return FALSE;

	BOOL ret = TRUE;
	UINT16 flags = 0;
	UINT16 seqId = Udp3SenderWindow_reserveDataSeq(&udp3->sendingWindow);
	udp3_write_headers(packet, udp3, UDPDATA_TYPE_DUMMY, seqId, TRUE, &flags);
	if ((flags & (RDPUDP2_ACKVEC | RDPUDP2_ACK)) == 0)
	{
		WLog_ERR(TAG, "possibly an error the generated packet doesn't have ACK or ACKVEC");
	}

	Stream_SealLength(packet);
	udp3_finalize_packet(packet);
	ret = rdpUdpTransport_bioSend(udp, packet);
	udp3->nextPing = now + MAX_PING_DELAY;
...
BOOL udp3_low_level_reader(rdpUdpTransport* udp, Udp3Context* udp3, wStream* s)
{
	...
	Stream_SetPositionUnchecked(s, 0);
	...
	{
		Udp3ReceiveWindow_treatAoA(udp3->recvWindow, packet.ackOfAcksSeqNum, now);
	}
	...
	if (packet.flags & RDPUDP2_DELAYACKINFO)
	{
		do
		{
			...
			if (packet.delayedAckTimeoutInMs < 10)
			...
			udp->timerInterval = packet.delayedAckTimeoutInMs;
		} while (FALSE);
	}

	if (packet.flags & RDPUDP2_ACK)
	{
		EnterCriticalSection(&udp3->sendingLock);
		if (!Udp3SenderWindow_treatAck(&udp3->sendingWindow, &packet, now))
		{
			// WLog_ERR(TAG, "error treating ACK");
		}
		LeaveCriticalSection(&udp3->sendingLock);
	}
	else if (packet.flags & RDPUDP2_ACKVEC)
	{
		EnterCriticalSection(&udp3->sendingLock);
		if (!Udp3SenderWindow_treatAckvec(udp, udp3, &packet, now))
		{
	...
	return checkForAckOrAckVec(udp, udp3, now);
}
...
BOOL udp3_low_level_writer(rdpUdpTransport* udp, Udp3Context* udp3, wStream* s)
{
	...
	{
		EnterCriticalSection(&udp3->sendingLock);
		UINT16 dataSeqId = Udp3SenderWindow_reserveDataSeq(&udp3->sendingWindow);

		udp3_write_headers(packet, udp3, UDPDATA_TYPE_DATA, dataSeqId, /*TRUE*/ FALSE, nullptr);
		...
		{
			LeaveCriticalSection(&udp3->sendingLock);
			WLog_ERR(TAG, "unable to create a payload packet");
			return FALSE;
		}

		UINT16 channelSeqId = Udp3SenderWindow_reserveChannelSeq(&udp3->sendingWindow);

		Stream_Write_UINT16(dataPayload, channelSeqId);
		Stream_Write(dataPayload, Stream_Pointer(s), toTake);
		Stream_Seek(s, toTake);

		ret = Udp3SenderWindow_Push(&udp3->sendingWindow, dataSeqId, channelSeqId, dataPayload);
		LeaveCriticalSection(&udp3->sendingLock);
		...
		Stream_SealLength(packet);
		udp3_finalize_packet(packet);
		udp3->nextPing = GetTickCount() + MAX_PING_DELAY;
		if (!rdpUdpTransport_bioSend(udp, packet))
			goto out;

		Stream_SetPositionUnchecked(packet, 0);
	}
...
static BOOL udp3_update_lost_packets(rdpUdpTransport* udp, Udp3Context* udp3, UINT64 now,
                                     size_t* nsent)
{
	Udp3SenderWindow* sendingWindow = &udp3->sendingWindow;
	BOOL ret = FALSE;
	...
	EnterCriticalSection(&udp3->sendingLock);
	UDP_DEBUG("udp3_update_lost_packets()");
	...
	*nsent = 0;
	for (UINT64 i = sendingWindow->lowIndex; i < upBound; i++)
	{
		...
		if (node->state == UDP3_PACKET_PENDING && now > node->sendTimestamp + LOST_TIMEOUT)
		{
			...
			if (node->channelSeqId)
			{
				UDP_DEBUG("sent packet 0x%lx (chanSeqId16=0x%x) is lost", i, node->channelSeqId);
				if(!udp3_resend_packet(udp, udp3, i))
				{
					WLog_ERR(TAG, "error resending packet seqId=0x%lx", i);
					goto out;
				}
				++*nsent;
				node->state = UDP3_PACKET_RECEIVED;
	...
	ret = TRUE;
out:
	Udp3SenderWindow_recomputeLowBound(sendingWindow, now);
	LeaveCriticalSection(&udp3->sendingLock);
...
{
	/* UDP_DEBUG("low=0x%lx up=0x%lx ackable=0x%lx", udp3->recvWindow->lowIndex,
	          udp3->recvWindow->upIndex, udp3->recvWindow->ackableIndex); */

	/* let's force delayInfo and AoA for timer packet */
	UINT64 now = GetTickCount64();
	if (udp3->sendingWindow.lastAoA + 200 < now || now >= udp3->nextPing)
	...
	{
		size_t nsent = 0;
		udp3_update_lost_packets(udp, udp3, now, &nsent);
		udp3->nextLostCheck = now + LOST_CHECK_INTERVAL;
	}

	BOOL ret = TRUE;

	if (now >= udp3->nextPing || shouldAck(udp3, now) || shouldAckVec(udp3, now))
	{
		wStream* s = rdpUdpTransport_getPacket(udp);
		if (!s)
			return FALSE;

		UDP_DEBUG("sending PING packet(delta now/nextPing=%ld)", now - udp3->nextPing);
		UINT16 dataSeqNumber = 0;
		UdpDataType dataType = UDPDATA_TYPE_NONE;
		UINT16 flags = 0;

		if (now >= udp3->nextPing)
		{
			flags |= RDPUDP2_DATA;
			dataType = UDPDATA_TYPE_DUMMY;
			dataSeqNumber = Udp3SenderWindow_reserveDataSeq(&udp3->sendingWindow);
		}
		...
		if (udp3->sendingWindow.pendingAoA)
			flags |= RDPUDP2_AOA;

		if (shouldAckVec(udp3, now))
			flags |= RDPUDP2_ACKVEC;
		else
			flags |= RDPUDP2_ACK;

		udp3_write_headersEx(s, udp3, now, dataType, dataSeqNumber, flags);
		udp3_finalize_packet(s);
		Stream_SealLength(s);
...
static BOOL treat_PacketPrefixByte(wStream* s, BOOL* isDummy)
{
	...
	BYTE packetPrefixByte = buf[7];
	buf[7] = buf[0];

	BYTE packetTypeIndex = (packetPrefixByte >> 1) & 0x0f;
	...
	*isDummy = (packetTypeIndex == 8);
	...
	Stream_Seek(s, 1);
	BYTE shortLen = (packetPrefixByte >> 5) & 0x7;
	if (shortLen != 7)
		Stream_SetLengthUnchecked(s, 1 + shortLen);
	return TRUE;
}
...
BOOL udp3_parse_packet(wStream* s, Udp3Packet* p)
{
	...
	if (p->flags & RDPUDP2_ACK)
	{
		...
		if (Stream_GetRemainingLength(s) < 7)
		{
			WLog_ERR(TAG, "%s: invalid ACK field (remaining=%ld)", __func__,
