                                      ...
                                      UINT16 channelSeqNumber16, wStream* payload)
{
	...
	{
		WLog_INFO(TAG, "%s: packet is out-of-window, lowIndex=0x%lx seqNumber=0x%lx", __func__,
	...
	{
		WLog_ERR(TAG,
		         "chunk 0x%.8lx(channelSeqNumber16=0x%x) is out of channelWindow [0x%.8lx -> "
		         "0x%.8lx], retransmit ?",
		         channelSeqNumber, channelSeqNumber16, w->lowChannelWindow, w->upChannelWindow);
		return TRUE;
	}
		...
		if (b & 0x80)
		{
			...
			for (j = 0; j < runLength; j++, seq++)
			{
				...
				switch (node->state)
				{
					...
					case UDP3_PACKET_RECEIVED:
						if (!status)
							WLog_ERR(TAG, "Ignoring lost status for already acked packet 0x%lx", seq);
						break;
					case UDP3_PACKET_LOST:
						if (status)
						{
...
static void pushAck(Udp3Context* udp3, wStream* s, UINT64 now)
{
	...
	if (nacks)
	{
		...
		UDP_DEBUG("%s: range=[0x%0.6lx -- 0x%0.6lx] ackable=0x%0.6lx acking16=0x%0.4lx (%d acks)",
