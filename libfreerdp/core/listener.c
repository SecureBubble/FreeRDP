                                     ...
                                     UINT16 port, BOOL udp)
{
	...
	struct addrinfo* res = freerdp_resolve_host(bind_address, port, sockType, ai_flags);
	if (!res)
		return FALSE;
	...
	for (struct addrinfo* ai = res; ai && (listener->num_sockfds < 5); ai = ai->ai_next)
	{
		...
		switch (ai->ai_family)
		{
			...
			case AF_INET6:
				...
				break;
			default:
				WLog_ERR(TAG, "family %d not supported", ai->ai_family);
...
static BOOL freerdp_listener_check_fds_ex(freerdp_listener* instance, DWORD waitResult)
{
	...
	for (int i = startIndex; i < listener->num_sockfds; i++)
	{
		(void)WSAResetEvent(listener->events[i]);

		if (WaitForSingleObject(listener->events[i], 0) != WAIT_OBJECT_0)
			continue;
	...
	if (startIndex < currentBound)
	{
		for (i = 0; i < listener->num_sockfds_udp; i++)
		{
			...
			Stream_SetLengthUnchecked(s, status);
