...
static MtParseResult multitransport_parse_one_message(multiTransportChannel* channel, wStream* s)
{
	...
	MtParseResult res = mt_tunnel_headers_parse(s, &action, &payloadLen, &subHeaderLen);
	if (res != MT_PARSE_OK)
		return res;
	...
	switch (action)
	{
		case RDPTUNNEL_ACTION_CREATEREQUEST:
		{
			...
			UINT32 reqId = Stream_Get_UINT32(payload);
		...
		case RDPTUNNEL_ACTION_CREATERESPONSE:
		{
			...
			INT32 hrResult = Stream_Get_INT32(payload);
			WLog_DBG(TAG, "createTunnelResponse(hrResult=%d)", hrResult);

			cbRet = channel->createTunnelResp(channel, hrResult);
			if (cbRet)
			{
				rdpContext* context = channel->multiTransport->rdp->context;
				UdpChannelEstablishedEventArgs args = { 0 };
				args.channel = channel;

				PubSub_OnUdpChannelEstablished(context->pubSub, context, &args);
			}
			break;
		}
...
static BOOL mtchannel_handlePackets(multiTransportChannel* channel)
{
	...
	size_t npackets = 0;
	BOOL doLoop = TRUE;
	while (doLoop && npackets < 10)
	{
		ReadPacketStatus result;
		wStream* packet = multitransportchannel_readPacket(channel, &result);
		switch (result)
		{
		...
		{
			BOOL innerLoop = TRUE;
			while (innerLoop && Stream_GetRemainingLength(packet))
			{
		...
		npackets++;
		// UDP_DEBUG("mtchannel_handlePackets: npackets=%zu treated", npackets);
	}
	return TRUE;
}
                                                        ...
                                                        BOOL lossy, const BYTE* cookie)
{
	...
	ret->bio =
	    rdpUdpTransport_init_client(settings, ret->tls, lossy, cookie, settings->ServerHostname,
	                                settings->ServerPort, &completed, &ret->transport);
