...
typedef struct
{
	...
	Udp3Node* range;
	size_t lowIndex;
	volatile LONG upIndex;

	volatile LONG channelSeqNumber;
...
typedef struct
{
	...
	UINT64 timebase;
	CRITICAL_SECTION sendingLock;
	Udp3SenderWindow sendingWindow;
	Udp3ReceiveWindow* recvWindow;
	UINT64 nextLostCheck;
	UINT64 nextPing;
	BOOL firstAckVec;
	UINT64 lastAckVecTime;
	UINT64 nextAckVecTime;
	UINT64 lastAckTime;
	UINT64 lastAckSent;
	UINT64 nextAckTime;
} Udp3Context;

#define NEXT_TIME_NEVER UINT64_MAX
