...
/* MS-TSGU packet types (mirror of rdg.c RdgPktType) */
...
#define HTTP_EXTENDED_AUTH_SSPI_NTLM 0x04

#define RDG_HEADER_LEN 8 /* type(2) + reserved(2) + packetLength(4) */

typedef enum
{
	WSTS_STATE_INIT,
	WSTS_STATE_HANDSHAKE,
	WSTS_STATE_EXTAUTH,
	WSTS_STATE_TUNNEL_CREATE,
	WSTS_STATE_TUNNEL_AUTH,
	WSTS_STATE_CHANNEL_CREATE,
	WSTS_STATE_OPENED
} WSTS_STATE;
...
struct rdp_wsts
{
	...
	rdpCredsspAuth* auth; /* server-side NTLM acceptor */
	UINT16 clientExtAuth;
...
static BOOL wsts_http_upgrade(rdpWsts* wsts)
{
	...
	/* request-target: "GET /remoteDesktopGateway?...&AuthS=SSPI_NTLM HTTP/1.1".
	 * The browser/RDS HTML5 web client uses websocket; the legacy RD Gateway
	 * HTTP transport (RDG_IN_DATA/RDG_OUT_DATA) is NOT supported here. */
	if (_strnicmp(buf, "GET ", 4) != 0)
	{
		WLog_Print(wsts->log, WLOG_ERROR,
		           "upgrade: not a GET (non-websocket RD Gateway HTTP transport?)");
		return FALSE;
	}
...
/* ----------------------------------------------------------------------------
 * MS-TSGU PDU send helpers (server replies). Layouts inverted from rdg.c
 * client parsers (rdg_process_*).
 * ------------------------------------------------------------------------- */

static BOOL wsts_send_pdu(rdpWsts* wsts, wStream* s)
{
	Stream_SealLength(s);
	const BOOL ok = wsts_ws_send(wsts, Stream_Buffer(s), Stream_Length(s), WebsocketBinaryOpcode);
	Stream_Free(s, TRUE);
	return ok;
}

static wStream* wsts_pdu_new(UINT16 type, size_t bodyLen)
{
	wStream* s = Stream_New(NULL, RDG_HEADER_LEN + bodyLen);
	if (!s)
		return NULL;
	Stream_Write_UINT16(s, type);
	Stream_Write_UINT16(s, 0);
	Stream_Write_UINT32(s, (UINT32)(RDG_HEADER_LEN + bodyLen));
	return s;
}

/* HANDSHAKE_RESPONSE: errorCode(4) verMajor(1) verMinor(1) serverVersion(2) extAuth(2) */
static BOOL wsts_send_handshake_response(rdpWsts* wsts)
{
	wStream* s = wsts_pdu_new(PKT_TYPE_HANDSHAKE_RESPONSE, 10);
	if (!s)
		return FALSE;
	Stream_Write_UINT32(s, 0);                            /* errorCode = S_OK */
	Stream_Write_UINT8(s, 1);                             /* VersionMajor */
	Stream_Write_UINT8(s, 0);                             /* VersionMinor */
	Stream_Write_UINT16(s, 0);                            /* ServerVersion */
	Stream_Write_UINT16(s, HTTP_EXTENDED_AUTH_SSPI_NTLM); /* offered ext auth */
	return wsts_send_pdu(wsts, s);
}

/* EXTENDED_AUTH: errorCode(4) authBlobLen(2) blob */
static BOOL wsts_send_extauth(rdpWsts* wsts, const BYTE* blob, UINT16 blobLen)
{
	wStream* s = wsts_pdu_new(PKT_TYPE_EXTENDED_AUTH_MSG, 6ull + blobLen);
	if (!s)
		return FALSE;
	Stream_Write_UINT32(s, 0); /* ERROR_SUCCESS */
	Stream_Write_UINT16(s, blobLen);
	if (blobLen)
		Stream_Write(s, blob, blobLen);
	return wsts_send_pdu(wsts, s);
}

/* TUNNEL_RESPONSE: serverVersion(2) errorCode(4) fieldsPresent(2) reserved(2) [tunnelId(4)][caps(4)] */
static BOOL wsts_send_tunnel_response(rdpWsts* wsts)
{
	const UINT16 fields = 0x1 /*TUNNEL_ID*/ | 0x2 /*CAPS*/;
	wStream* s = wsts_pdu_new(PKT_TYPE_TUNNEL_RESPONSE, 10ull + 4 + 4);
	if (!s)
		return FALSE;
	Stream_Write_UINT16(s, 0);      /* serverVersion */
	Stream_Write_UINT32(s, 0);      /* errorCode = S_OK */
	Stream_Write_UINT16(s, fields); /* fieldsPresent */
	Stream_Write_UINT16(s, 0);      /* reserved */
	Stream_Write_UINT32(s, 1);      /* tunnelId */
	Stream_Write_UINT32(s, 0);      /* capabilities (none required of client) */
	return wsts_send_pdu(wsts, s);
}

/* TUNNEL_AUTH_RESPONSE: errorCode(4) fieldsPresent(2) reserved(2) [redirFlags(4)][idleTimeout(4)] */
static BOOL wsts_send_tunnel_auth_response(rdpWsts* wsts)
{
	const UINT16 fields = 0x1 /*REDIR_FLAGS*/ | 0x2 /*IDLE_TIMEOUT*/;
	wStream* s = wsts_pdu_new(PKT_TYPE_TUNNEL_AUTH_RESPONSE, 8ull + 4 + 4);
	if (!s)
		return FALSE;
	Stream_Write_UINT32(s, 0);      /* errorCode = S_OK */
	Stream_Write_UINT16(s, fields); /* fieldsPresent */
	Stream_Write_UINT16(s, 0);      /* reserved */
	/* redirFlags: permissive (proxy is the policy point). The MS-TSGU redir bits are
	 * DISABLE_* flags (DRIVE=0x1 PRINTER=0x2 PORT=0x4 CLIPBOARD=0x8 PNP=0x10); 0x7F
	 * wrongly set DISABLE_CLIPBOARD so the web client stopped advertising cliprdr.
	 * HTTP_TUNNEL_REDIR_ENABLE_ALL (0x80000000) allows all redirections; let the
	 * inner RDP decide actual policy. TODO: derive from config. */
	Stream_Write_UINT32(s, 0x80000000); /* HTTP_TUNNEL_REDIR_ENABLE_ALL */
	Stream_Write_UINT32(s, 0);    /* idleTimeout = none */
	return wsts_send_pdu(wsts, s);
}

/* CHANNEL_RESPONSE: errorCode(4) fieldsPresent(2) reserved(2) [channelId(4)] */
static BOOL wsts_send_channel_response(rdpWsts* wsts)
{
	const UINT16 fields = 0x1 /*CHANNELID*/;
	wStream* s = wsts_pdu_new(PKT_TYPE_CHANNEL_RESPONSE, 8ull + 4);
	if (!s)
		return FALSE;
	Stream_Write_UINT32(s, 0);      /* errorCode = S_OK */
	Stream_Write_UINT16(s, fields); /* fieldsPresent */
	Stream_Write_UINT16(s, 0);      /* reserved */
	Stream_Write_UINT32(s, 1);      /* channelId */
	return wsts_send_pdu(wsts, s);
}

/* ----------------------------------------------------------------------------
 * NTLM acceptor (in-band EXTENDED_AUTH PDUs)
 * ------------------------------------------------------------------------- */

static BOOL wsts_auth_init(rdpWsts* wsts)
{
	wsts->auth = credssp_auth_new(wsts->context);
	if (!wsts->auth)
		return FALSE;
	/* NTLM, channel-bound to the outer TLS like the inner NLA path */
	if (!credssp_auth_init(wsts->auth, NTLM_SSP_NAME, wsts->tls->Bindings))
		return FALSE;
	return credssp_auth_setup_server(wsts->auth);
}

/* Process one inbound EXTENDED_AUTH PDU body (errorCode(4) blobLen(2) blob). */
static BOOL wsts_handle_extauth(rdpWsts* wsts, wStream* s)
{
	...
	Stream_Read_UINT16(s, blobLen);
	if (!Stream_CheckAndLogRequiredLength(TAG, s, blobLen))
		return FALSE;

	const BYTE* blob = Stream_ConstPointer(s);

	/* The proxy is the policy point and re-authenticates to the real backend with
	 * its own token, so we cannot (and need not) validate the user's NTLM password.
	 * winpr emits a well-formed CHALLENGE for the NEGOTIATE message (T1), but it
	 * cannot validate the AUTHENTICATE message (T3) without the user's hash. Accept
	 * T3 unconditionally: the tunnel is TLS-protected and MS-TSGU DATA PDUs are not
	 * NTLM-sealed, so the unused NTLM session key does not matter. */
	const BOOL isAuthenticate =
	    (blobLen >= 12) && (memcmp(blob, "NTLMSSP\0", 8) == 0) && (blob[8] == 0x03);
	if (isAuthenticate)
	{
		WLog_Print(wsts->log, WLOG_INFO,
		           "NTLM AUTHENTICATE accepted without validation (non-validating gateway)");
		if (!wsts_send_extauth(wsts, NULL, 0)) /* completion: errorCode=0, blobLen=0 */
			return FALSE;
		wsts->state = WSTS_STATE_TUNNEL_CREATE;
		return TRUE;
	}

	SecBuffer in = { 0 };
	in.cbBuffer = blobLen;
	in.pvBuffer = malloc(blobLen ? blobLen : 1);
	if (!in.pvBuffer)
		return FALSE;
	Stream_Read(s, in.pvBuffer, blobLen);
	credssp_auth_take_input_buffer(wsts->auth, &in); /* takes ownership */

	/* NEGOTIATE (T1): let winpr produce the CHALLENGE (T2). */
	if (credssp_auth_authenticate(wsts->auth) < 0)
	{
		WLog_Print(wsts->log, WLOG_ERROR, "NTLM challenge (AcceptSecurityContext on T1) failed");
		return FALSE;
	}

	if (credssp_auth_have_output_token(wsts->auth))
	{
		const SecBuffer* out = credssp_auth_get_output_buffer(wsts->auth);
		if (!wsts_send_extauth(wsts, (const BYTE*)out->pvBuffer, (UINT16)out->cbBuffer))
			return FALSE;
	}
	return TRUE;
}
...
/* ----------------------------------------------------------------------------
 * Handshake driver: run the PDU exchange until OPENED
 * ------------------------------------------------------------------------- */

static BOOL wsts_run_handshake(rdpWsts* wsts)
{
	wsts->state = WSTS_STATE_HANDSHAKE;

	while (wsts->state != WSTS_STATE_OPENED)
	{
		BYTE* payload = NULL;
		size_t len = 0;
		if (!wsts_ws_recv(wsts, &payload, &len))
			return FALSE;

		wStream sbuf = { 0 };
		wStream* s = Stream_StaticConstInit(&sbuf, payload, len);
		if (!Stream_CheckAndLogRequiredLength(TAG, s, RDG_HEADER_LEN))
		{
			free(payload);
			return FALSE;
		}
		...
		Stream_Read_UINT32(s, pktLen);

		BOOL ok = TRUE;
		switch (type)
		{
			case PKT_TYPE_HANDSHAKE_REQUEST:
				/* verMajor(1) verMinor(1) clientVersion(2) extendedAuth(2) */
				if (Stream_CheckAndLogRequiredLength(TAG, s, 6))
				{
					Stream_Seek(s, 4);
					Stream_Read_UINT16(s, wsts->clientExtAuth);
				}
				ok = wsts_send_handshake_response(wsts);
				if (ok)
				{
					if (wsts->clientExtAuth & HTTP_EXTENDED_AUTH_SSPI_NTLM)
					{
						ok = wsts_auth_init(wsts);
						wsts->state = WSTS_STATE_EXTAUTH;
					}
					else
						wsts->state = WSTS_STATE_TUNNEL_CREATE;
				}
			...
			case PKT_TYPE_EXTENDED_AUTH_MSG:
				ok = wsts_handle_extauth(wsts, s);
				break;

			case PKT_TYPE_TUNNEL_CREATE:
				ok = wsts_send_tunnel_response(wsts);
				if (ok)
					wsts->state = WSTS_STATE_TUNNEL_AUTH;
				break;

			case PKT_TYPE_TUNNEL_AUTH:
				/* body: fieldsPresent(2) clientNameLen(2) clientName(UTF16) — logged only */
				ok = wsts_send_tunnel_auth_response(wsts);
				if (ok)
					wsts->state = WSTS_STATE_CHANNEL_CREATE;
				break;

			case PKT_TYPE_CHANNEL_CREATE:
				/* body names the resource the client wants; the proxy chooses the
				 * real target, so we accept unconditionally. */
				ok = wsts_send_channel_response(wsts);
				if (ok)
					wsts->state = WSTS_STATE_OPENED;
				break;
			...
			default:
				WLog_Print(wsts->log, WLOG_WARN, "unexpected PDU type 0x%" PRIx16 " in handshake",
				           type);
				break;
		}
	...
	WLog_Print(wsts->log, WLOG_INFO, "MS-TSGU tunnel OPENED");
	return TRUE;
}
	...
	for (;;)
	{
		...
		if (wsts_try_frame(wsts, &payload, &plen, &opcode))
		{
			if ((opcode == WebsocketBinaryOpcode) || (opcode == WebsocketContinuationOpcode))
			{
				...
				{
					const UINT16 type = (UINT16)(payload[0] | (payload[1] << 8));
					if (type == PKT_TYPE_DATA)
					{
						const UINT16 dataSize = (UINT16)(payload[8] | (payload[9] << 8));
						const size_t off = RDG_HEADER_LEN + 2;
						size_t dlen = plen - off;
						if (dataSize < dlen)
							dlen = dataSize;
						if (dlen)
						{
							wsts->dataResidual = (BYTE*)malloc(dlen);
							if (!wsts->dataResidual)
							{
								free(payload);
								BIO_clear_flags(bio, BIO_FLAGS_SHOULD_RETRY);
								return -1;
							}
							memcpy(wsts->dataResidual, payload + off, dlen);
							wsts->dataResidualLen = dlen;
							wsts->dataResidualPos = 0;
						}
					}
					/* non-DATA (e.g. KEEPALIVE 0xD): drop and continue */
				}
				free(payload);
				continue; /* serve residual or parse the next frame */
			}
