                                                ...
                                                UINT16 sequenceNumber)
{
	...
	s = rdp_message_channel_pdu_init_ex(autodetect->context->rdp, &sec_flags, transport);
	if (!s)
		return FALSE;
	...
	return rdp_send_message_channel_pdu_ex(autodetect->context->rdp, transport, s,
                                                   ...
                                                   UINT16 sequenceNumber, UINT16 payloadLength)
{
	...
	wStream* s = rdp_message_channel_pdu_init_ex(autodetect->context->rdp, &sec_flags, transport);
	if (!s)
		return FALSE;
                                                ...
                                                const AUTODETECT_REQ_PDU* autodetectReqPdu)
{
	...
	return autodetect_send_rtt_measure_response(autodetect, transport,
